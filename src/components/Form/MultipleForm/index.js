...
import { Row, Col, Card, Input, Form, Select, Alert, Checkbox, DatePicker, Spin } from "antd";
            ...
            <Row gutter={16}>
              ...
              {datas.form &&
                ...
                datas.form.map((form, indexForm) => {
                  return (
                    <Col key={indexForm} className="gutter-row" span={form.colSpan}>
                      <Row gutter={16}>
                        {form.data &&
                          form.data.length > 0 &&
                          form.data.map((data, indexData) => {
                            return <RenderFormCheckbox key={indexData} data={data} />;
                          })}
                      </Row>
                    </Col>
                  );
        ...
        >
          <Fragment>
            ...
            {dataInput.type === "date" && (
              <DatePicker
                style={{ width: "100%" }}
                value={dataInput.value ? moment(dataInput.value, "YYYY-MM-DD") : ""}
            ...
            {dataInput.type === "select" && (
              <Select
                ...
                disabled={dataInput.disabled ? dataInput.disabled : false}
              >
                {dataInput.placeholder && <Option value="">{dataInput.placeholder}</Option>}
...
const RenderAutoComplete = memo(({ value, option, onChange, placeholder, isLabelValue }) => {
  ...
  useEffect(() => {
    if (useRefWrapperDropdown.current) {
      const handleScroll = (e) => {
        if (
          useRefWrapperDropdown.current.scrollTop + useRefWrapperDropdown.current.clientHeight >=
      ...
      {showDropdown && (
        <div className="dropdown-wrapper">
          <div className="area-dropdown">
            <input value={search} placeholder="Search..." onChange={handleChangeSearch} />

            <ul ref={useRefWrapperDropdown} style={{ maxHeight: 160, overflow: "auto" }}>
              {filterData.length > 0 &&
                filterData.map((d, i) => {
                  return (
                    <li key={i} onMouseDown={() => handleSelectValueAutoComplete(d.value, d.label)}>
                      {d.value} - {d.label}
                    </li>
                  );
                })}
