  ...
  constructor(props) {
    ...
    this.state = {
      ...
      dataDummy: {
        ...
        description:
          this.props.data.nomorDaftar +
          " / " +
          this.props.data.tanggalDaftar +
          "\n" +
      ...
      if (this.props.kodeDok == "BC 28" || this.props.kodeDok == "P3Bet") {
        for (var i = 0; i < this.props.childrenData.length; i++) {
          let data = {
            ...
            tanggal_daftar: moment(
              this.props.childrenData[i].tanggal_dokumen_dokasal,
              "YYYY-MM-DD"
            ).format("YYYY-MM-DD"),
            id: Math.floor(Math.random() * 90000) + 10000,
            name:
              this.props.childrenData[i].kode_jenis_dokumen_dokasal == "331"
                ? "P3Bet"
                : "BC " + this.props.childrenData[i].kode_jenis_dokumen_dokasal,
            description:
              this.props.childrenData[i].nomor_dokumen_dokasal +
              " / " +
              moment(this.props.childrenData[i].tanggal_dokumen_dokasal, "YYYY-MM-DD").format(
                "YYYY-MM-DD"
              ) +
              "\n" +
        ...
        for (var i = 0; i < this.props.childrenData.length; i++) {
          let data = {
            ...
            tanggal_daftar: moment(
              this.props.childrenData[i].tanggal_daftar_dok_asal,
              "YYYY-MM-DD"
            ).format("YYYY-MM-DD"),
            id: Math.floor(Math.random() * 90000) + 10000,
            name:
              this.props.childrenData[i].kode_jenis_dok_asal == "331"
                ? "P3Bet"
                : "BC " + this.props.childrenData[i].kode_jenis_dok_asal,
            description:
              this.props.childrenData[i].nomor_daftar_dok_asal +
              " / " +
              moment(this.props.childrenData[i].tanggal_daftar_dok_asal, "YYYY-MM-DD").format(
                "YYYY-MM-DD"
              ) +
              "\n" +
  ...
  getDataChildren = async (id, nomor, kode_kantor, tanggal_daftar, children, name) => {
    try {
      if (children == undefined) {
        ...
        if (name == "BC 25") {
          var requestBodyBc25 = esb.requestBodySearch().query(
            esb
              .boolQuery()
          ...
          if (responseBc25.data.message == "success") {
            if (responseBc25.data.data.hits.hits.length == 0) {
              ...
              if (responseBc25.data.data.hits.hits[0]._source.bahan_baku == undefined) {
                ...
                for (
                  let j = 0;
                  j < responseBc25.data.data.hits.hits[0]._source.bahan_baku.length;
                  j++
                ) {
                  var dataBc25 = {
                    nomor:
                      responseBc25.data.data.hits.hits[0]._source.bahan_baku[j]
          ...
          var requestBodyBc27 = esb.requestBodySearch().query(
            esb
              .boolQuery()
              .must(esb.matchQuery("nomor_daftar.keyword", "" + nomor + ""))
              .must(esb.matchQuery("kode_kantor.keyword", "" + kode_kantor + ""))
          ...
          if (responseBc27.data.message == "success") {
            if (responseBc27.data.data.hits.hits.length == 0) {
              ...
              if (responseBc27.data.data.hits.hits[0]._source.bahan_baku == undefined) {
                ...
                for (
                  let j = 0;
                  j < responseBc27.data.data.hits.hits[0]._source.bahan_baku.length;
                  j++
                ) {
                  var dataBc27 = {
                    nomor:
                      responseBc27.data.data.hits.hits[0]._source.bahan_baku[j]
                        .nomor_daftar_dok_asal,
                    kode_kantor:
          ...
          var requestBodyBc28 = esb.requestBodySearch().query(
            esb
              .boolQuery()
              .must(esb.matchQuery("nomor_daftar.keyword", "" + nomor + ""))
              .must(esb.matchQuery("kode_kantor.keyword", "" + kode_kantor + ""))
          ...
          if (responseBc28.data.message == "success") {
            if (responseBc28.data.data.hits.hits.length == 0) {
              ...
              if (responseBc28.data.data.hits.hits[0]._source.dokumen_asal == undefined) {
                ...
                for (
                  let j = 0;
                  j < responseBc28.data.data.hits.hits[0]._source.dokumen_asal.length;
                  j++
                ) {
                  var dataBc28 = {
                    nomor:
                      responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j]
                        .nomor_dokumen_dokasal,
                    kode_kantor:
                      responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j].kd_kantor_dokasal,
                    tanggal_daftar: moment(
                      responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j]
                        .tanggal_dokumen_dokasal,
                      "YYYY-MM-DD"
                    ).format("YYYY-MM-DD"),
                    id: Math.floor(Math.random() * 90000) + 10000,
                    name:
                      responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j]
                        .kode_jenis_dokumen_dokasal == "331"
                        ? "P3Bet"
                        : "BC " +
                          responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j]
                            .kode_jenis_dokumen_dokasal,
                    description:
                      responseBc28.data.data.hits.hits[0]._source.dokumen_asal[j]
                        .nomor_dokumen_dokasal +
                      " / " +
          ...
          var requestBodyBc41 = esb.requestBodySearch().query(
            esb
              .boolQuery()
              .must(esb.matchQuery("nomor_daftar.keyword", "" + nomor + ""))
              .must(esb.matchQuery("kode_kantor.keyword", "" + kode_kantor + ""))
          ...
          if (responseBc41.data.message == "success") {
            if (responseBc41.data.data.hits.hits.length == 0) {
              ...
              if (responseBc41.data.data.hits.hits[0]._source.bahan_baku == undefined) {
                ...
                for (
                  let j = 0;
                  j < responseBc41.data.data.hits.hits[0]._source.bahan_baku.length;
                  j++
                ) {
                  var dataBc41 = {
                    nomor:
                      responseBc41.data.data.hits.hits[0]._source.bahan_baku[j]
                        .nomor_daftar_dok_asal,
                    kode_kantor:
        ...
        .on("nodeClick", function (event) {
          if (event.type == "focus") {
            ...
            if (event.nodeDataItem.data.children.length == 0) {
              getDataChildren(
                event.nodeDataItem.data.id,
                event.nodeDataItem.data.nomor,
                event.nodeDataItem.data.kode_kantor,
                event.nodeDataItem.data.tanggal_daftar,
                event.nodeDataItem.data.childrenData,
                event.nodeDataItem.data.name
              );
            }
          }
        })
