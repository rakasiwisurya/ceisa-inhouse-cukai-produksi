...
export default class BRCK2Perbaikan extends Component {
  ...
  handleUpdate = async () => {
    ...
    const payload = {
      idBrck2: this.props.match.params.id,
      idNppbkc: idNppbkc,
      nppbkc: nppbkc,
      ...
      keteranganKekurangan:
        Math.sign(selisihLt) === -1 || Math.sign(selisihKemasan) === -1 ? selisihDescription : null,
      flagSanksi:
    ...
    const response = await requestApi({
      service: "produksi",
      method: "post",
      endpoint: endpoints.brck2Perbaikan,
      body: payload,
  ...
  render() {
    ...
    return (
      <>
        <Container menuName="Buku Rekening Cukai" contentName="BRCK-2 Perbaikan">
          <Row gutter={[16, 16]}>
            <Col span={12}>
