...
export default class PermohonanTarifTaskToDoPerbaikan extends Component {
  ...
  handleUploadFile = (e) => {
    this.setState({ tasktodoFile: e.target.files[0] });
  };
  ...
  handleSimpanTasktodo = async () => {
    const { tasktodoStatus, tasktodoAlasan, tasktodoFile } = this.state;
    ...
    const response = await requestApi({
      service: "produksi",
      method: "post",
      endpoint: endpoints.permohonanTarifPerbaikanTasktodo,
      body: formData,
